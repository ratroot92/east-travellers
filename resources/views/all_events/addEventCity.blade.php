    ...
    <div class="sb2-2">
        ...
        <div class="sb2-2-add-blog sb2-2-1">
            <div class="box-inn-sp">
                <div class="inn-title">
                    <h4>@if($action=='add') Add Event @else Update Event @endif City</h4>
                    <p>Airtport Hotels The Right Way To Start A Short Break Holiday</p>
                </div>
                <div class="bor">
                    @if($action == 'add')
                    <form method="post" enctype="multipart/form-data" action="{{route("view.add.city.post")}}">
                    @else
                    <form method="post" enctype="multipart/form-data" action="{{route("view.update.eventcity.post")}}">
                    <input type="hidden" name="id" id="id" value="{{ $Event_City->id }}">
                    @endif
                        {{csrf_field()}}
                        <div class="row">
                            <div class="input-field col s12">
                                <input id="" type="text" name="name" class="validate" @if($action=='add') @else value="{{ $Event_City->name ?? '' }}" @endif required="required"/>
                        ...
                        <div class="row">
                            ...
                            <div class="input-field col s12">
                                <div class="file-field">
                                    <div class="btn">
                                        <span>City Image <i class="fa fa-file-image-o"></i></span>
...
@else
 <div class="row">
                            ...
                            @if(isset($Event_City->image))
                            <img src="{{ $Event_City->image }}" class="img-fluid img-responsive" />
                            @else
...
@endif
                        <div class="row">
                            <div class="input-field col s12">
                                <textarea id="description" class="materialize-textarea" name="description" required="required">@if($action=='add') @else {{ $Event_City->description ?? '' }} @endif</textarea>
                                <label for="description">City Descriptions:</label>
                            </div>
                        </div>
